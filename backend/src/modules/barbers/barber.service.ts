...
export class BarberService {
  ...
  async updateSchedule(barberId: string, data: any) {
    // Validar que no exista conflicto con breaks
    const conflictingBreak = await prisma.break.findFirst({
      where: {
        barberId,
    ...
    if (conflictingBreak) {
      throw new AppError(409, 'Existe un descanso que conflicta con este horario');
    }
  ...
  // ── Agregar descanso ──
  async createBreak(barberId: string, data: any) {
    ...
    if (existingBreak) {
      throw new AppError(409, 'Ya existe un descanso en ese horario');
    }
    ...
    if (schedule) {
      ...
      if (bStart < wsStart || bEnd > wsEnd) {
        throw new AppError(400, 'El descanso debe estar dentro del horario laboral');
      }
    }
  ...
  // ── Eliminar descanso ──
  async deleteBreak(breakId: string, barberId: string) {
    const breakItem = await prisma.break.findFirst({
      where: { id: breakId, barberId },
    });

    if (!breakItem) {
      throw new AppError(404, 'Descanso no encontrado');
    }
  ...
  // ── Agregar día libre ──
  async createDayOff(barberId: string, data: any) {
    ...
    if (date < today) {
      throw new AppError(400, 'No puedes agregar días libres en el pasado');
    }
    ...
    if (existing) {
      throw new AppError(409, 'Ya existe un día libre en esta fecha');
    }
    ...
    if (appointments) {
      throw new AppError(409, 'No puedes agregar día libre: tienes citas agendadas');
    }
  ...
  // ── Eliminar día libre ──
  async deleteDayOff(dayOffId: string, barberId: string) {
    const dayOff = await prisma.dayOff.findFirst({
      where: { id: dayOffId, barberId },
    });

    if (!dayOff) {
      throw new AppError(404, 'Día libre no encontrado');
    }

    return prisma.dayOff.delete({
      where: { id: dayOffId },
    });
  }
}
