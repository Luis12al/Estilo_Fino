...
class AppointmentService {
    ...
    async getAvailability(query) {
        ...
        const [breaks, existingAppointments] = await Promise.all([
            database_1.prisma.break.findMany({
        ...
        // ── 4. Calcular slots ──
        const slots = this.calculateSlots(schedule.startTime, schedule.endTime, breaks, existingAppointments, durationMinutes, date);
        return {
    ...
    async getTodayStats(barberId) {
        ...
        const [total, pending, confirmed, inProgress, completed, cancelled] = await Promise.all([
            ...
            database_1.prisma.appointment.count({ where: { ...where, status: 'PENDING' } }),
            database_1.prisma.appointment.count({ where: { ...where, status: 'CONFIRMED' } }),
            database_1.prisma.appointment.count({ where: { ...where, status: 'IN_PROGRESS' } }),
            database_1.prisma.appointment.count({ where: { ...where, status: 'COMPLETED' } }),
            database_1.prisma.appointment.count({ where: { ...where, status: 'CANCELLED' } }),
        ]);
    ...
    async create(data, clientId) {
        ...
        if (services.length !== serviceIds.length) {
            throw new error_middleware_1.AppError(400, 'Uno o más servicios no existen');
        }
        const totalDuration = services.reduce((sum, s) => sum + s.durationMinutes, 0);
        const totalPrice = services.reduce((sum, s) => sum + Number(s.price), 0);
        const start = new Date(startTime);
        const end = new Date(start.getTime() + totalDuration * 60000);
        const appointment = await database_1.prisma.$transaction(async (tx) => {
            ...
            if (overlapping) {
                throw new error_middleware_1.AppError(409, 'Este horario ya fue reservado. Por favor selecciona otro.');
            }
            ...
            if (!workSchedule || !workSchedule.isActive) {
                throw new error_middleware_1.AppError(400, 'El barbero no trabaja en este horario');
            }
            ...
            if (start < workStart || end > workEnd) {
                throw new error_middleware_1.AppError(400, 'La cita debe estar dentro del horario laboral');
            }
            ...
            for (const b of breaks) {
                ...
                if (start < bEnd && end > bStart) {
                    throw new error_middleware_1.AppError(400, 'La cita coincide con un descanso del barbero');
                }
            }
            ...
            if (dayOff) {
                throw new error_middleware_1.AppError(400, 'El barbero tiene día libre en esta fecha');
            }
     ...
     */
    async createManual(data, barberId) {
    ...
    async updateStatus(appointmentId, barberId, status, reason) {
        ...
        if (!appointment) {
            throw new error_middleware_1.AppError(404, 'Cita no encontrada o no pertenece a este barbero');
        }
        const allowedNext = VALID_TRANSITIONS[appointment.status];
        if (!allowedNext.includes(status)) {
            throw new error_middleware_1.AppError(400, `Transición inválida: ${appointment.status} → ${status}. Permitidas: ${allowedNext.join(', ')}`);
        }
     ...
     */
    async extend(appointmentId, barberId, additionalMinutes = 20) {
        const appointment = await database_1.prisma.appointment.findFirst({
            where: { id: appointmentId, barberId, status: 'IN_PROGRESS' },
        });
        if (!appointment) {
            throw new error_middleware_1.AppError(404, 'Cita no encontrada o no está en progreso');
        }
        const newEndTime = new Date(appointment.endTime.getTime() + additionalMinutes * 60000);
        const conflicting = await database_1.prisma.appointment.findFirst({
            where: {
                barberId,
        ...
        if (conflicting) {
            throw new error_middleware_1.AppError(409, `No se puede extender ${additionalMinutes}min. Conflicto con cita a las ${this.formatTime(conflicting.startTime)}`);
        }
        const updated = await database_1.prisma.appointment.update({
            where: { id: appointmentId },
            data: {
    ...
    // ========== MÉTODOS PRIVADOS ==========
    calculateSlots(workStart, workEnd, breaks, appointments, durationMinutes, date) {
        const slots = [];
